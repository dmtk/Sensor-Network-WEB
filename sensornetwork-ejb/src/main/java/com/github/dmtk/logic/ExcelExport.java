...
import com.github.dmtk.dto.NetworkEventDTO;
...
public class ExcelExport {
    ...
    public File exportSpectrum (Collection<NetworkEvent> rows) throws IOException {
        return exportSpectrum("NetworkEvent", rows);
    }

    public File exportSpectrum (String header, Collection<NetworkEvent> rows) throws IOException {
        return writeSpectrum(header, createNewFile(), rows);
    }
        
    public File exportExperiments (Collection<NetworkEventDTO> rows) throws IOException {
    ...
    public File writeSpectrum(String header, File f, Collection <NetworkEvent> report) throws IOException {
        ...
        sheet = workbook.createSheet("NetworkEvents");
        writeSpectrumTable(header, sheet, report);

        return writeExcelFile(f, workbook);
    }
        
    public File writeExperiments(File f, Collection<NetworkEventDTO> report) throws IOException {
    ...
    private void writeSpectrumTable(String header, Sheet sheet, Collection<NetworkEvent> spectrum) {
        ...
        int rowNum = 4;
        for (Iterator<NetworkEvent> it = spectrum.iterator(); it.hasNext();) {
            NetworkEvent dataRow = it.next();
            Row row = sheet.createRow(rowNum);
            row.createCell(0).setCellValue(dataRow.getSource().getName());
            row.createCell(1).setCellValue(dataRow.getValue());
            rowNum++;
        }
    }
        
    private void writeExperimentsTable(Sheet sheet, Collection<NetworkEventDTO> experiments) {
        Row firstRow = sheet.createRow(0);
        ...
        int rowNum = 4;
        for (Iterator<NetworkEventDTO> it = experiments.iterator(); it.hasNext();) {
            NetworkEventDTO dataRow = it.next();
            Row row = sheet.createRow(rowNum);
